		...
		do {
			...
			case 1: {
				...
				if (size < n) {
					...
					case 1: // Teacher
						System.out.println("Enter city, name, college name, qualification:");
						person = new Teacher(++eventid, sc.next(), sc.next(), sc.next(), sc.next());
						break;
					case 2: // Student
						System.out.println("Enter  city, name, school name, percentage:");
			...
			case 3: // Get details specific participant
				...
				System.out.println("Enter id to find which id: ");
				...
				for (Person p : persons) {
					if (userId == p.id) {
						flag = true;
						// super obj check obj of which type
						if (p instanceof Student) {
							((Student) p).toLearn(); // down casting for super class to sub class
						} else {
							((Teacher) p).toTeach();
						}
					}
