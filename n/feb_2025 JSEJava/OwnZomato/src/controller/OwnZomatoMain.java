...
//		_______________________________________________________________________________________________________________________________
		...
		System.out.println(restservices.getRestrentsById("R201"));

		System.out.println("-=-=- User login -=-=-=");
		try {
			userservices.login("U101");
			restservices.getAllRestrents().stream().forEach(System.out::println);

			restservices.getAllRestrents().stream().forEach(rest -> {
				System.out.println(
						rest.getName() + "\t" + rest.getRating().cakculateRatingAverage() + "\t" + rest.getId());
				System.out.println("\n----- MENU -----");
				rest.getItems().stream().forEach(item -> {
					System.out.println(item.getName() + "\t ₹ " + item.getPrice());
				});
...
//		_______________________________________________________________________________________________________________________________
		restservices.rateRestaurent("FriendsCafe", 4, "Sukoon");
		restservices.rateRestaurent("dominoes", 2, "good");
		Restaurant rest = restservices.getFindByName("FriendsCafe");
		System.out.println("Name: " + rest.getName() + "\t rating: " + rest.getRating().cakculateRatingAverage()
				+ "\n comments:" + rest.getRating().getComments());
